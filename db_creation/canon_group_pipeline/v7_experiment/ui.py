...
def dashboard_data() -> dict:
    ...
    for result in report.get("results", []):
        grouped[result.get("game", "Unknown game")].append(result)
    games = []
...
.mapping{display:grid;grid-template-columns:minmax(170px,1fr) 45px minmax(170px,1fr) 150px 100px;gap:12px;align-items:center;padding:14px 16px;border-top:1px solid #1e3443}.mapping:first-child{border-top:0}.tag-label{font-size:9px;letter-spacing:.13em;text-transform:uppercase;color:var(--muted);margin-bottom:5px}.tag{font-weight:670}.arrow{text-align:center;color:#567184;font-size:20px}.context{display:inline-block;margin-top:6px;color:#9eb3c1;font-size:10px;background:#172a38;border:1px solid #2b4353;border-radius:99px;padding:3px 7px}.relation{font-weight:680}.relation small{display:block;color:var(--muted);font-weight:450;margin-top:5px}.meter{height:5px;background:#253743;border-radius:9px;overflow:hidden;margin-top:6px}.fill{height:100%;background:var(--violet)}.decision{text-align:center;font-size:10px;font-weight:800;letter-spacing:.09em;border-radius:7px;padding:8px 5px}.decision.accept{color:var(--green);background:#102e25;border:1px solid #286d52}.decision.reject{color:var(--red);background:#321d23;border:1px solid #753e47}.reason{grid-column:1/-1;color:var(--muted);font-size:12px;line-height:1.45;background:#0a1721;border-radius:7px;padding:8px 10px}.empty{text-align:center;border:1px dashed var(--line);border-radius:12px;padding:40px;color:var(--muted)}
